...
import {
  ...
  CloudRain, RefreshCw, CheckCircle, ArrowLeft, Eye,
} from 'lucide-react';
...
export default function Schedule() {
  ...
  // PASO 3: confirma y guarda definitivamente en BD
  const confirmMutation = useMutation({
    mutationFn: async () => {
      const res = await api.post(
        `/tournaments/${selectedTournament}/schedule/confirm`,
        {
          date:               selectedDate,
          courts:             courtConfigs,
          roundDurations,
  ...
  const ScheduleTable = ({ rows }: { rows: any[] }) => {
    ...
    return (
      <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ backgroundColor: '#F9FAFB' }}>
            {['#','Hora','Sede','Cancha','Ronda','Cat.','Jugador 1','Jugador 2','Dur.'].map(h => (
        ...
        <tbody>
          {rows.map((row: any, i: number) => (
            <tr key={i} style={{
              ...
              <td style={{ padding: '7px 9px', color: '#9CA3AF', fontSize: '11px' }}>{i + 1}</td>
              <td style={{ padding: '7px 9px', fontWeight: '700', color: row.status === 'suspended' ? '#F97316' : '#1B3A1B' }}>{row.time}</td>
              <td style={{ padding: '7px 9px', color: '#6B7280', fontSize: '11px' }}>{row.sede}</td>
      ...
      <main className="flex-1 p-8">
        ...
        {step === 2 && preview && (
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                <h2 style={{ ...h2, marginBottom: 0 }}>Detalle de partidos</h2>
                <div style={{ display: 'flex', gap: '6px' }}>
                  {[{ v: true, label: 'Por Sede' }, { v: false, label: 'Lista' }].map(({ v, label }) => (
                    <button key={label} onClick={() => setViewBySede(v)}
                      style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '12px', border: 'none', cursor: 'pointer', backgroundColor: viewBySede === v ? '#2D6A2D' : '#F3F4F6', color: viewBySede === v ? 'white' : '#374151' }}
        ...
        {step === 3 && published && (
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <ScheduleTable rows={published.schedule || []} />
            </div>
          </>
        )}
